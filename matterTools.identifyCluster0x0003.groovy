...
void identify(timeInSeconds){
    identify(identifyTime:timeInSeconds as Integer)
}
...
List<Map> processClusterResponse_0003(Map descMap){
	List<Map> hubEvents = []

	switch(descMap.attrInt) {
		case 0x0000: // identify Time REmaining
			hubEvents << [name:"identifyTimeRemaining", value: Integer.parseInt(descMap.value, 16), descriptionText: "${device.displayName} will identify itself for ${Integer.parseInt(descMap.value, 16)} more seconds", isStateChange:false]
			break
		default:
			break
	}
