...
List<Map> processClusterResponse_0006(Map descMap){
    List<Map> hubEvents = []

    switch(descMap.attrInt) {
        case 0x0000: //onOff
            newSwitchState = ((Integer.parseInt(descMap.value, 16)) ? "on" : "off")
            hubEvents << [name:"switch", value: newSwitchState, descriptionText: "${device.displayName} turned ${newSwitchState}", isStateChange:false]
            break
        case 0x4000: //GlobalSceneControl
            break
        case 0x4001: // onTime
            hubEvents << [name:"onTime", value: (Integer.parseInt(descMap.value, 16) / 10), units:"seconds", isStateChange:false]
            break
        case 0x4002: // OffWaitTime
            hubEvents << [name:"OffWaitTime", value: (Integer.parseInt(descMap.value, 16) / 10), units:"seconds", isStateChange:false]
            break
