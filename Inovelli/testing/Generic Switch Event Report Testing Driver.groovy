...
metadata {
    definition (name: "Generic Switch Event Report Test Driver for Inovelli", namespace: "matterTools", author: "jvm33") {
...
void parse(String nodeReportRawDescriptionString) {
    // log.info "Device ${device.displayName}: report (not decoded) ${nodeReportRawDescriptionString}"
	Map decodedNodeReportMap = parseDescriptionAsDecodedMap(nodeReportRawDescriptionString) // Using ASN.1 parser (below)
    log.info "Device ${device.displayName}: report (decoded) ${decodedNodeReportMap}"
}
...
void configure(){
	...
	// String cmd = 'he cleanSubscribe 0x01 0x0040 [ {"ep":"0xFFFF","cluster":"0xFFFFFFFF","attr":"0xFFFFFFFF"}, {"ep":"0xFFFF","cluster":"0xFFFFFFFF","evt":"0xFFFFFFFF"}]'
	
	// Subscribe to all event reports for the Generic Switch cluster
    // Change cluster to "0xFFFFFFFF" to subscribe to all reports for all clusters.
    String cmd = 'he cleanSubscribe 0x01 0x0040 [ {"ep":"0xFFFF","cluster":"0x3B","evt":"0xFFFFFFFF"}]'
	
    log.info "${device.displayName}: Subscribing to all device event reports."
...
////////////////   Lines below here are for parsing ASN.1 data ////////////////////////
/*
library (
        base: "driver",
        author: "jvm33",
